...
class plot_form(models.Model): 
	...
	total_par_marla = fields.Float(string="Total Per Marla")
	total_par_sqft = fields.Float(string="Total Per SQFT")
	rate_per_sqft = fields.Float(string="Rate Per SQFT")

	@api.onchange('plot_sq_ft','rate_per_sqft')
	def calculate_total_sq_ft(self):
		self.total_par_sqft = self.plot_sq_ft * self.rate_per_sqft

	@api.onchange('total_marla','plot_rate_marla')
	def calculate_total_marla(self):
		print "zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz"
		self.total_par_marla = self.total_marla * self.plot_rate_marla

	@api.onchange('total_par_marla','total_par_sqft')
	def calculate_total_marla_sqft_price(self):
		self.total_plot_price = self.total_par_marla + self.total_par_sqft
...
class plot_payment_history(models.Model):
	...
	def Ms_number(self):
		...
		plot_file_ms=self.env['sale.order'].search([])
		for x in plot_file_ms:
			invoice_ms_no=self.env['account.invoice'].search([('name','=',x.plot_registration_no)])
			for z in invoice_ms_no:
				z.plot_payment_history_tree=x.ms_new
