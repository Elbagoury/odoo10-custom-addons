...
class Hr_Employee(models.Model):
    ...
    certification_id = fields.One2many('hr.certification', 'certification_relation', string="Training")
...
class Iqama(models.Model):
    ...
    relegion = fields.Selection(string="Religion", selection=[('m', 'Muslim'), ('n', 'Non-Muslim'), ], required=False, )
    dob = fields.Date('Date of Birth')
    iqama_no = fields.Char("Iqama/Saudi ID No", required=True)
...
class Gosi(models.Model):
    ...
    iqama_no = fields.Char("Iqama/ Saudi ID No")
...
class Experience(models.Model):
    ...
    comp = fields.Char('Company')
    s_date = fields.Date('Start Date')
    e_date = fields.Date('End Date')
    detail = fields.Text('Details')
    experience_relation = fields.Many2one('hr.employee', string='Experience Relation')
