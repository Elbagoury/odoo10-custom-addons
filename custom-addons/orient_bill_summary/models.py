...
class orient_bill_summary(models.Model):
	...
	bl_nmb      = fields.Many2one('bill.num',string = "B/L Number")
...
# ====================punching data in orient summary on update button of wizard===============
	...
	def update(self):
		...
		records = self.env['ufc.auto'].search([('customer.id','=',self.customer.id),('bl_number','=',self.bl_nmb.name)])
